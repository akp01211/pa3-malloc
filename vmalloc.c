...
static void footer_w(struct block_header *b){
 ...
 void *vmalloc(size_t size){
    ...
    size_t nec =ROUND_UP(size +sizeof(struct block_header),BLKALIGN);
 
    // Best fit alg
    struct block_header *best = NULL;
    for(struct block_header *b = heapstart; b->size_status !=VM_ENDMARK; b =block_n(b)){
    ...
    if(rem >=2* BLKALIGN){
        ...
        struct block_header *split= block_n(best);
        split->size_status =rem |VM_PREVBUSY;
        footer_w(split);
 
        struct block_header *next =block_n(split);
        ...
        struct block_header *nxt= block_n(best);
